...
def getNum(lst):
#        print(lst)
        r = [i for i in range(len(lst)) if lst[i] == 1.0]
        #print(r[0])
        print(r)
        return r
...
##	th1 = cv2.resize(gray, (150,150))
	...
	if (len(contours) > 0):
        	...
        	th1 = cv2.resize(gray, (150,150))
        	th2 = image.img_to_array(th1)
        	npa = np.array([th2])
        	predicted_classes = model.predict(npa)
        	#print(predicted_classes.flatten().tolist())
        	r = getNum(predicted_classes.flatten().tolist())
        	cv2.imshow("images", np.hstack([frame,skin]))
        	HCI2.HCI(r)
	...
	#cv2.imwrite('None/none' + str(i) + '.png',gray)
